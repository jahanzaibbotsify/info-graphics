  ...
  methods: {
    ...
    handleImageDownload() {
      try {
        ...
        const imageUrl = `${process.env.VUE_APP_BACKEND_URL || 'http://localhost:3000'}${this.infographic.imageUrl}`;
        
        // Use the download endpoint instead of direct image URL
        const link = document.createElement('a')
        link.href = `${process.env.VUE_APP_BACKEND_URL || 'http://localhost:3000'}/download/${this.infographic.imageFilename || 'infographic.png'}`
